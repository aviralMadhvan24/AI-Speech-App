...
import { MAX_WPM, ZONE_ARCS, getZoneColor } from "../../utils/paceZones";
 ...
 * Ported from speedometer/src/components/Speedometer.jsx, retains the original
 * visual fidelity (chrome bezel + zone arcs + glowing needle) on a transparent
 * background so it sits cleanly on top of our glass cards.
 */
export function SpeedometerGauge({ wpm, isAverage = false }: SpeedometerGaugeProps) {
  ...
  useEffect(() => {
    ...
    const draw = (value: number) => {
      ...
      bezel.addColorStop(0, "#3f3f46"); // zinc-700
      bezel.addColorStop(0.5, "#09090b"); // zinc-950
      bezel.addColorStop(1, "#27272a"); // zinc-800
      ctx.beginPath();
  ...
  const color = getZoneColor(Math.round(wpm));
  ...
  return (
    <div className="relative flex flex-col items-center select-none">
      <canvas
        ...
        aria-label="Speaking pace gauge"
      />
      <div className="absolute bottom-6 flex flex-col items-center pointer-events-none">
        <div
          className="text-7xl md:text-8xl font-bold tabular-nums tracking-tight transition-colors"
          style={{
            color,
        ...
        <div className="mt-1 text-xs uppercase tracking-widest text-zinc-500">
          {isAverage ? "Avg words / min" : "Words / min"}
        </div>
      </div>
    </div>
  );
