...
def _user(user_id: str, name: str = "Student", role: str = "student"):
    """A users-log row as the ranking reads it.

    The ranking needs both identifiers: the id to join debates and GD, the
    address to join submissions and drills. These stubs use the same string for
    both so a test can key its fixtures on one value — what is under test here
    is the ranking maths, not the resolution between the two.
    """
    return SimpleNamespace(
        firebase_uid=user_id, email=user_id, display_name=name, role=role
    )
...
def test_students_without_scored_work_are_not_ranked(buddy_env):
    buddy_env.users.append(_user("u-quiet"))
    assert service.rank_speakers() == []


def test_teachers_are_never_ranked(buddy_env):
    buddy_env.users.append(_user("u-teacher", role="teacher"))
    buddy_env.submissions["u-teacher"] = [_submission(content=90.0)]
    assert service.rank_speakers() == []


def test_unavailable_results_are_skipped_not_scored_as_zero(buddy_env):
    """An outage must not drag a student's average down."""
    buddy_env.users.append(_user("u-ada"))
    buddy_env.submissions["u-ada"] = [
...
def test_content_and_pronunciation_are_weighted_equally(buddy_env):
    buddy_env.users.append(_user("u-ada"))
    buddy_env.submissions["u-ada"] = [_submission(content=90.0, pronunciation=70.0)]
...
def test_content_only_student_is_still_rankable(buddy_env):
    buddy_env.users.append(_user("u-ada"))
    buddy_env.submissions["u-ada"] = [_submission(content=88.0)]
...
def test_ranking_is_ordered_best_first(buddy_env):
    buddy_env.users.extend([_user("u-low"), _user("u-high")])
    buddy_env.submissions["u-low"] = [_submission(content=60.0)]
    buddy_env.submissions["u-high"] = [_submission(content=95.0)]

    assert [r.email for r in service.rank_speakers()] == ["u-high", "u-low"]


def test_ranking_counts_active_mentees_only(buddy_env):
    buddy_env.users.append(_user("u-ada"))
    buddy_env.submissions["u-ada"] = [_submission(content=80.0)]
    buddy_env.pairs.create("u-ada", "u-bob", created_by_id="u-t")
    ended = buddy_env.pairs.create("u-ada", "u-cleo", created_by_id="u-t")
...
def test_one_good_day_is_not_enough_to_be_suggested(buddy_env):
    ...
    buddy_env.users.append(_user("u-ada"))
    buddy_env.submissions["u-ada"] = [_submission(content=95.0)]
...
def test_consistent_strong_speaker_is_suggested(buddy_env):
    buddy_env.users.append(_user("u-ada", name="Ada"))
    buddy_env.submissions["u-ada"] = [
        _submission(content=80.0),
    ...
    assert [s.email for s in suggested] == ["u-ada"]
...
def test_already_decided_students_drop_out_of_the_suggestions(buddy_env):
    ...
    buddy_env.users.extend([_user("u-ada"), _user("u-bob")])
    strong = [_submission(content=85.0), _submission(content=85.0)]
    buddy_env.submissions["u-ada"] = strong
    buddy_env.submissions["u-bob"] = strong

    buddy_env.mentors.set_status("u-ada", "approved", decided_by_id="u-t")
    buddy_env.mentors.set_status("u-bob", "rejected", decided_by_id="u-t")

    assert service.suggested_mentors() == []
    # They stay visible in the full ranking, carrying their decision.
    statuses = {r.email: r.status for r in service.rank_speakers()}
    assert statuses == {"u-ada": "approved", "u-bob": "rejected"}
...
def test_only_members_and_teachers_can_access_a_pair(buddy_env):
    pair = buddy_env.pairs.create("u-mentor", "u-mentee", created_by_id="u-t")

    assert service.can_access_pair(pair.pair_id, "u-mentor") is True
    assert service.can_access_pair(pair.pair_id, "u-mentee") is True
    assert service.can_access_pair(pair.pair_id, "u-stranger") is False
    assert (
        service.can_access_pair(pair.pair_id, "u-stranger", is_teacher=True) is True
    )


def test_unknown_pair_is_inaccessible_even_to_a_teacher(buddy_env):
    assert service.can_access_pair("no-such-pair", "u-t", is_teacher=True) is False
...
def _climber(env, email: str = "u-ada"):
...
def test_a_student_who_climbed_is_suggested_despite_a_low_lifetime_average(buddy_env):
    ...
    _climber(buddy_env)
    _closed_cycle(buddy_env, "u-ada", verdict="improved", axes=[(45.0, 72.0)])
...
def test_growth_alone_is_not_enough_if_they_are_still_weak(buddy_env):
    ...
    buddy_env.users.append(_user("u-bob"))
    buddy_env.submissions["u-bob"] = [
        _submission(content=20.0),
        _submission(content=35.0),
    ]
    _closed_cycle(buddy_env, "u-bob", verdict="improved", axes=[(20.0, 35.0)])
...
def test_a_small_climb_is_not_treated_as_evidence(buddy_env):
    ...
    _climber(buddy_env)
    _closed_cycle(buddy_env, "u-ada", verdict="improved", axes=[(66.0, 69.0)])
...
def test_only_a_verdict_of_improved_counts(buddy_env):
    ...
    for verdict in ("held", "declined", "not_enough_evidence"):
        ...
        _closed_cycle(buddy_env, "u-ada", verdict=verdict, axes=[(45.0, 72.0)])
...
def test_the_best_cycle_is_used_not_the_latest(buddy_env):
    ...
    _climber(buddy_env)
    _closed_cycle(buddy_env, "u-ada", verdict="improved", axes=[(45.0, 72.0)])
    _closed_cycle(buddy_env, "u-ada", verdict="improved", axes=[(70.0, 74.0)])
...
def test_a_strong_speaker_who_also_climbed_is_labelled_both(buddy_env):
    ...
    _closed_cycle(buddy_env, "u-ada", verdict="improved", axes=[(60.0, 85.0)])
...
def test_growth_candidates_sort_ahead_of_score_candidates(buddy_env):
    ...
    buddy_env.users.extend([_user("u-strong"), _user("u-climber")])
    buddy_env.submissions["u-strong"] = [
        _submission(content=92.0),
        _submission(content=92.0),
    ]
    buddy_env.submissions["u-climber"] = [
        _submission(content=40.0),
        _submission(content=72.0),
    ]
    _closed_cycle(buddy_env, "u-climber", verdict="improved", axes=[(45.0, 72.0)])
...
def test_growth_evidence_is_shown_even_when_it_changes_nothing(buddy_env):
    ...
    _closed_cycle(buddy_env, "u-ada", verdict="improved", axes=[(70.0, 79.0)])
...
def test_a_rejected_climber_is_not_re_suggested(buddy_env):
    ...
    _climber(buddy_env)
    _closed_cycle(buddy_env, "u-ada", verdict="improved", axes=[(45.0, 72.0)])
    buddy_env.mentors.set_status("u-ada", "rejected", decided_by_id="u-t")
...
def test_someone_elses_cycle_does_not_credit_this_student(buddy_env):
    _climber(buddy_env)
    _closed_cycle(buddy_env, "u-someone-else", verdict="improved", axes=[(45.0, 72.0)])
